...
class LidarSubscriber(Node):
    ...
    def draw_points(self, p, robot_gridx: int, robot_gridy: int) -> None:
        ...
        for point in p:
            # Occupied
            grid_x = int(point[0] / self.map_meta.resolution)
            grid_y = int(point[1] / self.map_meta.resolution)
    ...
    def transform_cloud(self, p, robot_x, robot_y, robot_heading):
        """
        Transform the point cloud by translation and rotation.

        Args:
            p (np.array): The point cloud data.
            robot_x (float): The robot's x position.
            robot_y (float): The robot's y position.
            robot_heading (float): The robot's heading.

        Returns:
            p (np.array): The transformed point cloud data.
        """
        # Translation
        p[:, 0] += robot_x
        p[:, 1] += robot_y
        ...
        # Rotation
        R = np.array([[np.cos(robot_heading), -np.sin(robot_heading)],
    ...
    def scan_callback(self, scan: LaserScan) -> None:
        ...
        p = self.transform_cloud(p, self.robot_x, self.robot_y, self.robot_heading)
