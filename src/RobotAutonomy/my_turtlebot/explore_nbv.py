...
class PRM(Node):
    def __init__(self) -> None:
        ...
        # PRM parameters
        self.num_nodes = 200
        self.num_neighbors = 2  # Number of nearest neighbors
    ...
    def map_callback(self, msg: SUB_MSG) -> None:
    ...
    def node_information(self, node, plot=False) -> int:
        """
        Calculate the information gain by computing the unknown,
        free, and obstacle cells in the map.
        ...
        # print(f"[PRM] {node}: {unknown_cells}")
    ...
    def plot_cell_types(self, node, unknown_cells, free_cells) -> None:
        """
        Plot the cell types reached by the robot.
    ...
    def node_distance(self, node1, node2) -> float:
        ...
        Args:
            node1 (Tuple[float, float]): The first node position
            node2 (Tuple[float, float]): The second node position
    ...
    def navigate_to_cell(self, cell) -> None:
        ...
        print("[NBV] Navigation done.")
    
    def next_best_view(self) -> None:
        """
        Find the next best view node by maximizing the information gain.
        """
        print("[NBV] Finding next best view node...")
        self.generate_random_nodes(self.num_views)
        max_gain = 0
        best_node = None
        for node in self.nodes:
            info = self.node_information(node)
            weight = self.node_distance(node, self.current_node)
            temp = info*np.exp(-self.lambda_ * weight)
            # print(f"Node {node}, temp: {temp}, weight: {weight}")
            gain = self.prev_gain + info*np.exp(-self.lambda_ * weight)
            if gain > max_gain:
                max_gain = gain
                best_node = node
        print(f"[NBV] Next best view node: {best_node} with gain {max_gain}")
        # self.plot()
        self.navigate_to_cell(best_node)
    ...
    def node_is_unknown(self, node) -> bool:
        ...
        if self.map_data[y * self.map_width + x] == -1:
            # print(f"Node {node} is in unknown space.")
            return True
        return False
    ...
    def find_k_nearest_neighbors(self, nodes, k) -> dict:
        """
    ...
    def connect_k_nearest_neighbors(self) -> None:
        """
        Finds indices of the K nearest neighbors for each node.
    ...
    def create_k_nearest_edges(self) -> None:
        """
    ...
    def add_start_and_goal(self, start, goal) -> None:
        """
    ...
    def dilate_map(self, map_data, clearance) -> list:
        ...
        return dilated_map.ravel().tolist()
