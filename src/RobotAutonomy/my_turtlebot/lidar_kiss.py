...
class LidarSubscriber(Node):
    ...
    def publish_transform(self, T):
        """
        Publish the transformation matrix as a TF. TODO
        """
        tfs = TransformStamped()
        tfs.header.stamp = self.get_clock().now().to_msg()
        tfs.header.frame_id = "world"
        tfs.child_frame_id = "lidar"
        tfs.transform.translation.x = T[0, 2]
        tfs.transform.translation.y = T[1, 2]
        tfs.transform.translation.z = 0.0

        r = R.from_matrix(T[:2, :2])
    ...
    def sub_callback(self, scan):
        ...
        while i < max_iter:
            ...
            if np.abs(prev_error - mean_error) < CONVERGE_TOLERANCE:
                ...
                # self.publish_transform(T_overall) # TODO
                break
